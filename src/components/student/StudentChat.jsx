...
export default function StudentChat() {
  ...
  const contactColor = searchParams.get('contactColor');
  ...
  useEffect(() => {
    ...
    const subUsers = supabase.channel('public:users:student_chat')
  ...
  useEffect(() => {
    if (contactId && contacts.length > 0) {
      const foundInFirebase = contacts.find(c => c.id === contactId || c.uid === contactId);
      if (foundInFirebase) {
      ...
      return (
        <div className="relative mt-2">
          ...
          <div className="absolute top-2 right-2 z-10">
            {isDubbing[msg.id] ? (
              <div className="bg-emerald-500 text-white text-xs px-3 py-1.5 rounded-full font-bold shadow-lg flex items-center gap-2 animate-pulse">
  ...
  return (
    <div className="h-screen w-full max-w-[1600px] mx-auto pt-[90px] pb-24 px-4 sm:px-6 flex gap-6">
      ...
      >
        ...
        <div className="p-4">
          <div className="relative">
            ...
            <input 
              type="text" 
              placeholder="Rechercher camarade ou prof..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-11 pr-4 py-3 bg-white/5 border border-white/10 rounded-2xl outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-all text-white placeholder-slate-500 text-sm"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar px-2 pb-2">
          {filteredContacts.map((contact) => (
            <div 
              ...
              className={`group flex items-center gap-4 p-3 rounded-2xl cursor-pointer transition-all duration-200 mb-1 ${(activeChat?.id === contact.id || activeChat?.uid === contact.uid) ? 'bg-white/10 shadow-lg' : 'hover:bg-white/5'}`}
            >
              <div className="relative">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white font-bold text-lg shadow-inner ${contact.avatarColor || 'bg-emerald-500'}`}>
                  {contact.name ? contact.name.charAt(0).toUpperCase() : '?'}
                </div>
                {contact.unread > 0 && (
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-emerald-500 rounded-full border-2 border-[#0B0F19] flex items-center justify-center text-[10px] font-bold text-white">
                    {contact.unread}
                  </span>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-1">
                  <h3 className="text-white font-semibold text-sm truncate">{contact.name} <span className="text-[10px] text-emerald-400 ml-1">{contact.role === 'teacher' ? '(Prof)' : ''}</span></h3>
                  <div className="flex items-center gap-2">
                    <span className={`text-[11px] font-medium ${contact.unread > 0 ? 'text-emerald-400' : 'text-slate-500'}`}>
                      {contact.role === 'teacher' ? 'Professeur' : 'Élève'}
                    </span>
                  </div>
                </div>
                <p className={`text-xs truncate ${contact.unread > 0 ? 'text-emerald-300 font-medium' : 'text-slate-500'}`}>
                  {contact.lastMessage}
                </p>
              </div>
            </div>
      ...
      >
        <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
          <div className="absolute top-[20%] right-[30%] w-[40%] h-[40%] bg-emerald-500/5 rounded-full blur-[100px]" />
        </div>
        ...
        {activeChat ? (
          <>
            <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-white/5 backdrop-blur-md z-10">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold shadow-inner ${activeChat.avatarColor || 'bg-emerald-500'}`}>
                  {activeChat.name ? activeChat.name.charAt(0).toUpperCase() : '?'}
                </div>
                <div>
                  <h2 className="text-white font-bold">{activeChat.name}</h2>
                  <p className="text-emerald-400 text-xs font-medium">En ligne {activeChat.role === 'teacher' ? '• Professeur' : ''}</p>
                </div>
              </div>
                  ...
                  return (
                    <motion.div key={msg.id} initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] ${isMe ? 'order-2' : 'order-1'}`}>
                        <div className={`px-5 py-3 rounded-3xl relative shadow-lg ${isMe ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-br-sm' : 'bg-white/10 border border-white/5 text-slate-200 rounded-bl-sm backdrop-blur-md'}`}>
                          {msg.text && <p className="text-sm leading-relaxed">{msg.text}</p>}
                          {renderMedia(msg)}
                        </div>
                        <div className={`flex items-center gap-1 mt-1 text-[10px] text-slate-500 ${isMe ? 'justify-end' : 'justify-start'}`}>
                          <span>{msg.time}</span>
                          {isMe && (msg.status === 'read' ? <CheckCheck className="w-3.5 h-3.5 text-emerald-400" /> : <Check className="w-3.5 h-3.5" />)}
                        </div>
                      </div>
                    </motion.div>
                  );
            ...
            <div className="p-4 border-t border-white/5 bg-white/5 backdrop-blur-xl z-10 rounded-b-[32px] relative">
              ...
              <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                ...
                <div className="flex-1 relative">
                  {isRecording ? (
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-full pl-5 pr-12 py-3.5 text-white placeholder-slate-500 text-sm outline-none focus:border-emerald-500/50 focus:bg-white/10 transition-all disabled:opacity-50"
                    />
                  )}
                </div>
                ...
                {isRecording ? (
                  ...
                  <button type="button" onMouseDown={startRecording} disabled={isUploading} className="w-12 h-12 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center hover:bg-emerald-500/30 transition-colors shrink-0 disabled:opacity-50">
                    <Mic className="w-5 h-5" />
                  </button>
                ) : (
                  <motion.button initial={{ scale: 0.8 }} animate={{ scale: 1 }} type="submit" disabled={isUploading} className="w-12 h-12 rounded-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.5)] hover:scale-105 transition-all shrink-0 disabled:opacity-50 disabled:hover:scale-100">
          ...
          <div className="flex-1 flex flex-col items-center justify-center z-10 p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">Messagerie Élève</h2>
            <p className="text-slate-400 max-w-sm">Sélectionnez un camarade ou un professeur dans la liste pour commencer à discuter ou poser des questions sur un cours.</p>
          </div>
        )}
      </motion.div>
